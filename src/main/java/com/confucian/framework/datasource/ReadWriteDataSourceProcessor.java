 ...
 * @author Zhang Kaitao
 */
public class ReadWriteDataSourceProcessor implements DestructionAwareBeanPostProcessor, PriorityOrdered {
	/**
	 * logger
	 */
	 ...
	 * 写操作方法map
	 */
	private Map<String, Boolean> writeMethodMap = new HashMap<String, Boolean>();
	...
	private class ReadWriteDataSourceTransactionException extends NestedRuntimeException {
		 ...
		 * sid
		 */
		private static final long serialVersionUID = 583386076344158407L;
	...
	private boolean isChoiceReadDB(String methodName) {
		...
		if (!StringUtils.isBlank(bestNameMatch)) {
			return false;
		}
	...
	/*
    order 以下两个方法，是为了兼容 shiro DefaultAdvisorAutoProxyCreator 而添加的，有待与测试验证
     */
	@Override
